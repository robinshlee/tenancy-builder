      ...
      <main className="max-w-4xl mx-auto px-6 py-10">
        ...
        {!loadError && agreements.length > 0 && (
          <ul className="divide-y divide-neutral-200 border border-neutral-200 rounded-lg bg-white">
            {agreements.map((a) => (
              <li key={a.id}>
                <Link href={`/agreements/${a.id}`} className="block px-5 py-4 hover:bg-neutral-50">
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`ml-4 text-xs font-medium px-2 py-1 rounded-full capitalize ${statusStyles[a.status] ?? "bg-neutral-100 text-neutral-700"}`}
                    >
                      {a.status}
                    </span>
                  </div>
